// Dungeon Master - Complete RPG Game Generator
class DungeonGame {
    constructor() {
        this.hero = null;
        this.monster = null;
    ...
    init() {
        this.showToast('Welcome to Dungeon Master! Ìºô', 'success');
        this.bindEvents();
    ...
    enterPlayMode() {
        ...
        this.showToast('Entered Play Mode! Ìºô', 'success');
    ...
    startAdventure() {
        ...
        this.showToast('Good luck, adventurer! Ìºô', 'success');
    }
    ...
    performCombat() {
        ...
        if (this.gameHP <= 0) {
            this.addCombatLog(`${this.hero.name} has fallen! Ìºô`);
            this.showToast('You have been defeated!', 'error');
            this.exitPlayMode();
        } else if (this.monster.hp <= 0) {
            this.addCombatLog(`${this.monster.name} defeated! Ìºô`);
            ...
            this.addCombatLog(`Combat continues! You have ${this.gameHP || 100} HP remaining.`);
        }
    }
    ...
    attemptFlee() {
        const fleeChance = 0.6 + (this.hero.stats.dex - 10) * 0.05;
    ...
    restInGame() {
        ...
        this.updateGameUI();
        this.addCombatLog(`Rested and healed ${healAmount} HP. Current HP: ${this.gameHP || 100}/100`);
        this.showToast(`Healed ${healAmount} HP! Ìºô`, 'success');
    ...
    searchRoom() {
        ...
        const searchChance = 0.4 + (this.hero.stats.wis - 10) * 0.03;
    ...
    triggerTrap() {
        ...
        if (this.gameHP <= 0) {
            ...
            this.showToast('You have been defeated by a trap! Ìºô', 'error');
            this.exitPlayMode();
        } else {
            this.addCombatLog(`You have ${this.gameHP || 100} HP remaining.`);
        }
    }
    ...
    collectTreasure() {
        ...
        this.showToast(`Treasure collected! Ìºô`, 'success');
...
const cellSize = rect && rect.width && rect.height ? Math.min(rect.width, rect.height) / Math.max(this.dungeonSize, 5) : 50;
    ...
    generateMonster() {
        ...
        this.displayMonster(this.monster);
        this.showToast(`Spawned monster: ${name}! Ìºô`, 'warning');
    ...
    generateStory() {
        ...
        this.showToast('New story generated! Ìºô', 'info');
    }
    ...
    startCombat() {
        ...
        if (this.hero.hp <= 0) {
            this.showToast(`${this.hero.name} has fallen! Ìºô`, 'error');
            this.addStoryEntry(`${this.hero.name} was defeated by ${this.monster.name}`, 'error');
        } else if (this.monster.hp <= 0) {
            this.showToast(`${this.monster.name} defeated! Ìºô`, 'success');
